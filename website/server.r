...
shinyServer(function(input, output, session) {
    ...
    if (input$plot_type == "scatter") {
      output$scatter_plot <- renderPlotly({
        ggplotly(ggplot(aes(y = atac, x = rna), data = data_raw) +
          ...
          layout(
            yaxis = list(title = "<b>ATAC-seq Abundance</b>"),
            xaxis = list(title = "<b>Gene expression</b>"),
            showlegend = FALSE,
            annotations = list(
              x = max(data_raw$rna),
              y = max(data_raw$atac),
              text = paste0(
                "R", " = ", round(cor(data_raw$rna, data_raw$atac, method = "spearman"), 3),
      ...
      output$scatter_plot <- renderPlotly({
        ggplotly(ggplot(aes(y = atac, x = rank), data = data_raw) +
          ...
          layout(
            yaxis = list(title = "<b>ATAC-seq Abundance</b>"),
            xaxis = list(title = "<b>Rank of gene expression</b>"),
            showlegend = FALSE,
            annotations = list(
              x = max(data_raw$rank),
              y = max(data_raw$atac),
              text = paste0(
                "R", " = ", round(cor(data_raw$rank, data_raw$atac, method = "spearman"), 3),
  ...
  observeEvent(input$generate_button_idr, {
    ...
    output$idr_plot_rank <- renderPlotly({
      ggplotly(ggplot(aes(x = log(V15), y = log(V19), color = idr), data = atac_idr_raw) +
        ...
        layout(
          yaxis = list(title = "<b> Replicate 2 signal (log)<b> "),
    ...
    output$idr_plot_sig <- renderPlotly({
      ggplotly(ggplot(aes(x = rank1, y = rank2, color = idr), data = atac_idr_raw) +
        ...
        scale_color_manual(values = c("indianred", "gray23"))) %>%
        layout(
          yaxis = list(title = "<b> Replicate 2 rank<b> "),
          xaxis = list(title = "<b> Replicate 1 rank<b> "),
          legend = list(title = list(text = "<b> IDR < 0.05 : </b>"), x = 0.2, y = 1.5, orientation = "h")
        ) %>%
        toWebGL()
    })
  ...
  datasetInput <- reactive({
    switch(input$idr_dataset,
           "IDR" = atac_idr_raw[, 1:12],
           "Replicate 1" = rename(atac_idr_raw[atac_idr_raw$Score<=540, c(1,4,13:16)],c('Chr'="Chr",'Name'="Name","Start"='V13','End'="V14",'Signal'="V15",'Summit'="V16")),
           "Replicate 2" = rename(atac_idr_raw[atac_idr_raw$Score<=540, c(1,4,17:20)],c('Chr'="Chr",'Name'="Name",'Start'="V17",'End'="V18",'Signal'="V19",'Summit'="V20")))
  })

    # Table of selected dataset ----
  output$idr_table <-  DT::renderDT(server = FALSE,{
     datasetInput()
  })


 output$idr_downloadData <-
    downloadHandler(
      filename = function () {
        paste(as.data.frame(input$idr_dataset),".csv", sep = "")
      },
